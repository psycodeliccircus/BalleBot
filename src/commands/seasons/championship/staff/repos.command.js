...
export default {
  ...
  run: async ({ message }) => {
    ...
    for (const group of groups) {
      try {
        const { collaborators } = await githubRequests.getCollaborators(
          group.repo
        );
        const { commits } = await githubRequests.getCommits(group.repo);
        await message.channel.send({
          embed: getRepoInfoTemplate({ group, collaborators, commits }),
        });
      } catch (error) {
        console.log(error);
        await message.channel.send(
          `Ocorreu um erro para buscar os dados da equipe ${group.name}`
        );
      }
    }
  },
