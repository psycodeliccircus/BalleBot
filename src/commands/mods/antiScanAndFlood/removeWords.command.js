  ...
  run: ({ message, client, args }) => {
    ...
    if (guildIdDatabase.has('wordsBanned')) {
      ...
      message.channel.send(message.author, new Discord.MessageEmbed()
        .setColor('#ff8997')
        .setThumbnail(client.user.displayAvatarURL({ dynamic: true }))
        .setTitle(`${message.author.tag} O seu servidor não possui um banco para excluir palavras e links proibidos`)
        .setDescription(`Para usar o comando primeiro adicione palavras no banco com o ${prefix}addwords <palavra1> <palavra2> <palavra3> etc`))
      return
    }
  }
}
