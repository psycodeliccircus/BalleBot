    ...
    if (!message.author.bot && message.content.startsWith(prefix)) {
      ...
      try {
        ...
        if (commandToBeExecuted) {
          if (message.channel.type === 'dm' && commandToBeExecuted.dm) return commandToBeExecuted.run({ client, message, args })

          const guildIdDatabase = new client.Database.table(`guild_id_${message.guild.id}`)
          const rolesPermissions = guildIdDatabase.get('admIds') || { owner: message.guild.ownerID };

          const rolesUser = client.guilds.cache.get(message.guild.id)
