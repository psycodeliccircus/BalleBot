...
export function messageWarnAndMute(message, client, messageMarked) {
  ...
  function messageDmAlert() {
    message.author.send(
      new Discord.MessageEmbed()
        ...
        .setDescription(`**Espere um moderador rever seu caso, por hora você está silenciado do servidor!**
            \n**Essa foi a mensagem:**
            \n ${messageMarked}`)
        .addFields({ name: 'Mensagem enviada no canal:', value: message.channel })
        .setTimestamp()
    )
  }
  ...
  message.channel.send(message.author, new Discord.MessageEmbed()
    .setColor('YELLOW')
    .setTitle(`${message.author.tag} você usou uma palavra ou link proibido e recebeu +1 warn, não use novamente ou será banido⚠️`));
  ...
  if (guildIdDatabase.has('channel_log')) {
    let channel = client.channels.cache.get(guildIdDatabase.get('channel_log'))

    if (channel) {
      channel.send(new Discord.MessageEmbed()
        .setColor('YELLOW')
        .setTitle(`Usuário ${message.author.tag} enviou uma mensagem suspeita:`)
        .setTimestamp()
